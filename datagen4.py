...
class DataGen():
    def __init__(self):
        ...
        self.save_directory = '/root/ocrtoc_ws/src/rendering_setup_iros22/trans_data'
    ...
    def save_data(self, img, pcd, R,  each_model_path, rotation , translation):
        ...
        trans_mat_file = os.path.join(folder, "transl.txt") 
        trans_mat = open(trans_mat_file, "w+")
        trans = np.array(translation)
        # print(type(trans), len(trans))
        # for row in trans:
        np.savetxt(trans_mat, trans)
        trans_mat.close()
        ...
        for roll in range(0, 360, 90):
            for pitch in range(0, 360, 45):
                for yaw in range(0, 360, 20):
                    ...
                    x_rand = round(random.uniform(-0.2, 0.2), 3)
                    
                    y_rand = round(random.uniform(-0.2, 0.2), 3)
                
                    z_rand = round(random.uniform(-0.2, 0.2), 3)
                    ...
                    translation = [x_rand, y_rand, z_rand]
                    rotation = [roll_rand,pitch_rand,yaw_rand]
                    pcd_r, R = self.pcd_rotate_setup(rotation)
                    # pcd_t = pcd_r.translate((0.0, 0.2, 0))
                    pcd_t = pcd_r.translate((translation[0], translation[1], translation[2]))
                    T = translation
                    print("translation", T)
                    img = self.camera_render_setup(pcd_t)
                    self.save_data(img, pcd_t, R, each_model_path, rotation, T)
